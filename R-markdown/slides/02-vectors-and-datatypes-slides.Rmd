...
`<-` is the assignment operator. It assigns values on the right to objects on the left. So, after executing `x <- 3`, the value of `x` is `3`. The arrow can
be read as 3 **goes into** `x`.  You can also use `=` for assignments but not in all contexts so it is good practice to use `<-` for assignments. `=` should only
...
other function names (e.g., `c`, `T`, `mean`, `data`, `df`, `weights`). 
...
inputs called *arguments*. Functions often (but not always) return a *value*. 

## Example of a function 

A typical example would be the function `sqrt()`. The input (the argument) must be
